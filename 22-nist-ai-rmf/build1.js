...
<nav class="sidebar">
  ...
  <div class="nav-group-title">Expert Tools</div>
  ...
  <div class="nav-group-title">Calculators</div>
  ...
  Object.keys(groups).forEach(function(fn){
    ...
    groups[fn].forEach(function(s){
      ...
      ['pass','partial','fail','na'].forEach(function(v){
        ...
        var active=st===v;
        html+='<button data-sid="'+s.id+'" data-sv="'+v+'" onclick="checkSetFromBtn(this)" style="border:1px solid '+(active?colors[v]:'#30363d')+';background:'+(active?colors[v]+'22':'transparent')+';color:'+(active?colors[v]:'#8b949e')+';border-radius:4px;padding:2px 8px;cursor:pointer;font-size:11px;">'+labels[v]+'</button>';
      });
      html+='</div>';
    });
    html+='</div>';
...
function checkSetFromBtn(btn){checkSet(btn.getAttribute('data-sid'),btn.getAttribute('data-sv'));}
